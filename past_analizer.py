...
# read csv data
df = pd.read_csv(read_file_path, header=None, usecols=[0, 2, 4, 6], dtype={'0':'float', '2':'float', '4':'float', '6':'float'})
df = df.rename(columns={0:'time', 2:'time_count', 4:'pump_state', 6:'flow'})
...
# grouping by minute
df_new = df.groupby(['year','month', 'day', 'hour', 'minute'], as_index=False).agg({'flow(ml)':'sum'})
